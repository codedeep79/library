...
THREE.DaydreamController = function () {
	...
	this.update = function () {
		...
		if (gamepad !== undefined && gamepad.pose !== undefined) {
			...
			if (
				...
				!angularVelocity.equals(pose.angularVelocity)
			) {
				angularVelocity.fromArray(pose.angularVelocity);
				scope.dispatchEvent({
					type: "angularvelocitychanged",
			...
			if (touchpadIsPressed !== gamepad.buttons[0].pressed) {
				...
				scope.dispatchEvent({
					type: touchpadIsPressed ? "touchpaddown" : "touchpadup",
				});
			}
	...
	this.getTouchPadState = function () {
		console.warn(
			"THREE.DaydreamController: getTouchPadState() is now getTouchpadState()"
		);
		return touchpadIsPressed;
	};
};
